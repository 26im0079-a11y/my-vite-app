...
const LUCK_DATA = [
  ...
  { fortuneJa: '大器晩成', fortuneEn: 'Late Bloomer (Awaiting Luck)', weight: 1, btnJa: '牙を研ぐ', btnEn: 'Sharpen Your Mind', commentsJa: ['今はシステムデバッグ中（凶）の如く苦戦するも、これより先、全てのバグは綺麗に解消され、驚くべき大躍進を遂げる運気なり。気力を蓄えよ。'], commentsEn: ['Though you struggle now like a system under debugging, all errors will soon clear away, leading to a massive leap forward. Gather your energy.'] },
...
const toSeiNoJi = (num: number): string => {
  ...
  if (remainder === 1) result += '一';
  else if (remainder === 2) result += '┬';
  else if (remainder === 3) result += '下';
  else if (remainder === 4) result += '兵'; 
  return result;
};
...
const toKanjiNumber = (num: number): string => {
  ...
  let res = ''; const tens = Math.floor(num / 10); const ones = num % 10;
  if (tens > 0) { if (tens > 1) res += kanjiNums[tens]; res += '十'; }
...
export default function App() {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('omikuji_history_v3');
    const savedDates = localStorage.getItem('omikuji_dates_v3');
    if (savedHistory) setHistory(JSON.parse(savedHistory));
    if (savedDates) setLastDates(JSON.parse(savedDates));
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.key === ' ' || e.key === 'Enter') && !isRolling && !showModal) {
        e.preventDefault(); drawOmikuji();
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [isRolling, lang, history, lastFortune, showModal]);
  ...
  const playSound = (type: 'roll' | 'success') => {
    try {
      ...
      if (type === 'roll') {
        ...
        gain.gain.setValueAtTime(0.08, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.5);
      } else {
        osc.type = 'sine'; osc.frequency.setValueAtTime(880, ctx.currentTime);
        gain.gain.setValueAtTime(0.12, ctx.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.6);
      }
  ...
  const drawOmikuji = () => {
    setIsRolling(true); setResult(null); playSound('roll');
    
    const today = new Date(); const hours = today.getHours(); const minutes = today.getMinutes();
    const month = today.getMonth() + 1; const date = today.getDate(); const dayOfWeek = today.getDay();

    let dateStr = lang === 'ja' 
      ? `令和${toKanjiNumber(today.getFullYear() - 2018)}年${toKanjiNumber(month)}月${toKanjiNumber(date)}日`
      : today.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    setVisitDate(dateStr);

    setTimeout(() => {
      let fortune = ''; let comment = ''; let currentBtnJa = '再び紐解く'; let currentBtnEn = 'Draw Again'; let historyKey = '';
      const timeStr = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;

      // ⏰ A. 特殊時間の判定
      if (SPECIAL_LUCK[timeStr]) {
        const spec = SPECIAL_LUCK[timeStr];
        fortune = lang === 'ja' ? spec.ja : spec.en; comment = lang === 'ja' ? spec.commentJa : spec.commentEn;
        currentBtnJa = '奇跡を重ねる'; currentBtnEn = 'Keep the Miracle'; historyKey = spec.ja;
      } 
      // 🎁 B. 祝日・記念日の判定
      ...
      // 🎲 C. 通常おみくじ（🔄 連続回避ロジック搭載）
        ...
        const weathers = [
          ...
          { ja: '曇天の雲の隙間から一条の光が差す如く、', en: 'Like a ray of sunlight piercing through cloudy data layers, ' }
        ];
        const weather = weathers[Math.floor(Math.random() * weathers.length)];
        const greeting = MONTHLY_GREETINGS[month] ? (lang === 'ja' ? MONTHLY_GREETINGS[month].ja : MONTHLY_GREETINGS[month].en) : '';
        
        comment = greeting + (lang === 'ja' ? weather.ja : weather.en) + (lang === 'ja' ? selectedGroup.commentsJa[0] : selectedGroup.commentsEn[0]);
      ...
      localStorage.setItem('omikuji_history_v3', JSON.stringify(newHistory));
      localStorage.setItem('omikuji_dates_v3', JSON.stringify(newDates));
    }, 600);
  };

  const executeClear = () => {
    localStorage.removeItem('omikuji_history_v3');
    localStorage.removeItem('omikuji_dates_v3');
    localStorage.removeItem('omikuji_visit_days');
    localStorage.removeItem('omikuji_last_visit');
    setHistory({}); setLastDates({}); setVisitDays(1); setResult(null); setLastFortune('');
    setShowModal(false);
  };
  ...
  const getTitle = () => {
    ...
    if (visitDays >= 3) return lang === 'ja' ? '【電脳参拝者】' : ' [Digital Pilgrim]';
    return '';
  };
  ...
  return (
    ...
    >
      ...
      <div className="max-w-md w-full bg-stone-50 rounded-lg shadow-2xl p-8 border-4 border-red-700 text-center relative overflow-hidden mt-8">
        ...
        <div className="min-h-[250px] flex flex-col items-center justify-center bg-stone-100 rounded border border-stone-300 p-6 mb-6 shadow-inner">
          {isRolling ? (
            ...
            <div className={`font-serif w-full ${lang === 'en' ? 'text-xs' : 'text-sm'}`}>
              <div className="text-3xl sm:text-4xl font-black text-red-700 mb-4 tracking-widest border-b-2 border-red-700/20 pb-2 px-4 inline-block animate-fade-in-up">
                {result.fortune}
              </div>
              <p className="text-stone-700 leading-relaxed text-left px-2 font-sans tracking-wide mb-4 animate-fade-in-up" style={{ animationDelay: '0.15s', animationFillMode: 'both' }}>
                {result.comment}
              </p>
              
              <div className="border-t border-dashed border-stone-300 pt-3 mt-2 font-serif text-stone-600 bg-stone-50/50 py-3 rounded flex flex-col gap-1 px-4 text-left animate-fade-in-up" style={{ animationDelay: '0.3s', animationFillMode: 'both' }}>
        ...
        <button
          onClick={drawOmikuji}
          disabled={isRolling}
          className={`w-full py-4 px-6 text-lg font-bold text-stone-100 rounded shadow-md transition-all duration-300 active:scale-98 tracking-widest font-serif focus:outline-none focus:ring-2 focus:ring-red-700 ${
      ...
      <div className="max-w-md w-full mt-6 bg-stone-50 rounded border border-stone-300 p-4 shadow-md font-serif text-center relative">
        <h3 className="text-xs font-bold text-stone-600 tracking-wider border-b border-stone-200 pb-1.5 mb-2.5 flex justify-between items-center px-1">
          <span className="flex items-center gap-1">
            ...
            <span className="text-[10px] text-red-700 font-bold font-sans">{getTitle()}</span>
          </span>
          <span className="text-[10px] font-normal text-stone-400 font-sans">
            {lang === 'ja' ? `成就：${completionRate}% / 参拝：${visitDays}日目` : `Progress: ${completionRate}% / Days: ${visitDays}`}
          </span>
        </h3>
          ...
          <div>
            <div className="flex flex-wrap justify-center gap-1.5 text-[10px] mb-3">
              {FORTUNE_ORDER.map(luckKey => {
                ...
                const lastDate = lastDates[luckKey];
                if (!count) return null;
                return (
                  <span 
                    key={luckKey} 
                    title={lang === 'ja' ? `最終拝受日: ${lastDate}` : `Last Received: ${lastDate}`}
                    className="bg-red-50 text-red-800 border border-red-200/60 rounded px-2 py-0.5 shadow-sm cursor-help transition-all hover:bg-red-100 relative group"
                  >
                    {luckKey} : <strong className="text-xs font-sans">{lang === 'ja' ? toSeiNoJi(count) : count}</strong>
                  </span>
                );
              })}
            </div>
            <button 
              ...
              className="text-[9px] text-stone-400 hover:text-red-700 underline transition-colors font-sans block mx-auto focus:outline-none focus:ring-1 focus:ring-red-700 rounded px-1"
            >
              {lang === 'ja' ? '御神籤をお焚き上げする' : 'Perform Sacred Disposal'}
            </button>
          </div>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-stone-50 border-2 border-red-800 max-w-xs w-full rounded p-6 shadow-2xl font-serif text-center relative animate-fade-in-up">
            <h4 className="text-lg font-bold text-red-800 mb-3">{lang === 'ja' ? '御神籤お焚き上げ' : 'Sacred Disposal'}</h4>
            <p className="text-xs text-stone-600 leading-relaxed mb-6">
              {lang === 'ja' ? 'これまでの参拝履歴、ならびに仮想御朱印をすべてお焚き上げ（消去）いたします。よろしいですか？' : 'This will cleanly burn and clear all your fortune logs and visited days. Do you proceed?'}
            </p>
            <div className="flex gap-3 justify-center text-xs">
              <button 
                onClick={executeClear}
                className="bg-red-800 text-stone-100 font-bold px-4 py-2 rounded hover:bg-red-900 transition-colors focus:outline-none focus:ring-2 focus:ring-red-700"
              >
                {lang === 'ja' ? 'お焚き上げする' : 'Burn and Clear'}
              </button>
              <button 
                onClick={() => setShowModal(false)}
                className="bg-stone-200 text-stone-700 font-bold px-4 py-2 rounded hover:bg-stone-300 transition-colors focus:outline-none focus:ring-2 focus:ring-stone-400"
              >
                {lang === 'ja' ? '取りやめる' : 'Cancel'}
              </button>
            </div>
          </div>
        </div>
